...
| `assistant_mode` | bool | `true` | Use `assistant.threads.setStatus` for concise high-level status text. Requires an AI-app Slack app with `assistant:write`; set to `false` to keep emoji-reaction status. It does not expose agent text deltas. |
| `streaming` | bool | `true` | Retained for config compatibility. Slack public-reply mode always buffers the turn and sends only the final user-facing answer; raw agent deltas are never posted or edited into Slack. |

> Slack public-reply mode also overrides `[reactions] narration_display` and visible tool detail. The thread status stays generic, and the final reply contains only public answer text plus the run link, Model, and Agent fields.
