...
#[tokio::test]
async fn deleting_profile_keeps_session_snapshot_and_marks_deleted() {
    let env = AdminTestEnv::new().await;
    let profile_service = env.profile_service();
    profile_service
    ...
    let deleted = deleted_response.json::<Value>().await.expect("delete json");
    assert_eq!(deleted["deleted"], true);
    ...
    let detail = client
        .get(format!(
            "{}/api/v1/sessions/{}",
            server.base_url,
            urlencoding::encode("slack:profile-thread")
        ))
        .bearer_auth(&env.token)
        .send()
        .await
    ...
    let row = list
        .iter()
        .find(|row| row["session_id"] == "slack:profile-thread")
        .expect("session row");
    assert_eq!(row["profile_id"], "codex-main");
    assert_eq!(row["profile_name"], "Codex Main");
    assert_eq!(row["profile_status"], "deleted");
    ...
    if let Some(profiles) = profiles["profiles"].as_array() {
        assert!(profiles.iter().all(|profile| profile["id"] != "codex-main"));
    }

    let base_config = AgentConfig::default();
    let base_options = HashMap::new();
    let err = profile_service
